...
## 2026-09-09 — 앱 버전 1.0.0 → 1.3.0 (스캐폴딩 이후 처음 버전 반영)

**배경**: GitHub Release 노트를 정리하다가 릴리즈 제목이 계속 `v1.0.0`인데 그 사이 삭제 기능·네이티브 랜딩·지원자 확인 화면 같은 새 기능이 여러 번 추가된 걸 발견 — 스캐폴딩 이후 `app.json`/`package.json`의 `version`을 한 번도 안 올리고 있었음.

**버전 결정**: `1.3.0`. 스캐폴딩(`1.0.0`) 이후 새 기능 3개(모집글/커뮤니티글 삭제, 네이티브 랜딩 화면, 지원자 확인·수락·거절 화면)를 각각 MINOR 1씩으로 보고 `1.0`→`1.3`, 그 사이 디자인 개편·버그 수정은 PATCH 성격이라 `1.3.0`에서 다시 0부터.

**중요 — runtimeVersion 정책과의 관계**: `app.json`의 `runtimeVersion.policy`가 `"appVersion"`이라, 버전을 올리면 그 자체로 새 `runtimeVersion`이 됨. 이 값은 빌드 시점에 네이티브 바이너리에 박혀서 OTA로는 못 바꿈 — 즉 **버전 변경은 새 EAS 빌드가 필수**이고, 이전 버전(`1.0.0`) 빌드는 그 시점부터 새 runtimeVersion(`1.3.0`)으로 발행되는 OTA 업데이트를 더 이상 받을 수 없음(기존 사용자는 반드시 새 APK를 재설치해야 함).

**검증**: `npx tsc --noEmit`, `npx expo export --platform web` 통과.

**다음 할 일**:
- [ ] `eas build -p android --profile internal`로 새 APK 빌드
- [ ] GitHub Release 태그/제목/노트를 `v1.3.0` 기준으로 정리, 새 APK로 자산 교체

---

## 2026-09-04 (4) — 오래 방치한 세션 만료 시 401에 갇히던 문제 수정

**신고된 증상**: 로그인 안 한 상태처럼 보이는데도 로그인 상태로 들어가지고, 마이페이지 진입 시 401 에러가 나는 것 같다는 제보. 실제로는 리로드하니 자연 복구됨.

**원인**: Supabase JS의 `autoRefreshToken` 타이머는 앱이 포그라운드에 있을 때만 도는데, RN은 웹과 달리 앱이 백그라운드로 가면 이 타이머가 그냥 멈춰버림(웹 브라우저 탭은 이 문제가 없음). 기기를 오래 켜두고 방치하면 로컬에 저장된 세션의 access token이 만료된 채로 남고, `getSession()`은 만료 여부와 무관하게 그 값을 그대로 반환하니 앱은 "로그인된 것처럼" 보이지만 실제 API 호출은 401로 실패함.

**수정**
- `src/server/supabase.ts`: Supabase 공식 권장 패턴대로 `AppState`를 감시해서, 앱이 포그라운드(`active`)로 돌아올 때마다 `supabase.auth.startAutoRefresh()`를 명시적으로 다시 걸고, 백그라운드로 가면 `stopAutoRefresh()`로 정리.
- `app/(app)/dashboard.tsx`: 그래도 401이 나는 경우를 대비해, `error.status === 401`을 감지하면 "다시 시도" 버튼이 있는 막다른 에러 화면 대신 **즉시 자동 로그아웃 → 로그인 화면 이동**으로 처리(재시도로는 절대 못 풀리는 상태였음).

**검증**: `npx tsc --noEmit`, `npx expo export --platform web` 통과. 실기기에서 리로드로 우연히 복구되는 것도 확인(정상 상황에선 AppState 갱신이 먼저 돌아서 이 시나리오 자체가 안 생겨야 함).

---

## 2026-09-04 (3) — 버튼 텍스트가 세로로 살짝 잘리던 버그 (`h-auto` 누락)

**신고된 증상**: 랜딩 화면 "둘러보기" 버튼 글자(특히 "ㄹ")의 가운데가 살짝 잘려 보임.

**원인**: `Button` 컴포넌트의 기본 사이즈가 `h-10`(40px) 고정인데, 여러 화면에서 `py-3`(세로 padding 24px)만 추가하고 높이는 안 덮어써서 `padding + 텍스트 줄높이`가 40px 박스에 다 안 들어가 위아래가 살짝 잘리던 문제. 로그인/회원가입 입력창에서 겪었던 것과 같은 유형의 버그(`h-10`+추가 padding 충돌)가 `Button`에도 있었음.

**수정**: `py-3`를 쓰면서 높이를 따로 안 정한 버튼 7곳 전부에 `h-auto` 추가 — 랜딩 3개(시작하기/둘러보기/프로젝트 시작하기), 로그인, 회원가입, 모집 지원하기, 404 화면 버튼(`recruit/[id].tsx`, `login.tsx`, `signup.tsx`, `landing.tsx`, `not-found-state.tsx`).
